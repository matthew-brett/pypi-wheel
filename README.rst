#####################################
Building and uploading tornado wheels
#####################################
...
https://travis-ci.org/matthew-brett/pypi-wheel

The driving github repository is
https://github.com/matthew-brett/pypi-wheel
...
* builds a tornado wheel, linking against these fresh builds;
...
* making a commit to the `pypi-wheel` repository (e.g. with `git commit
  --allow-empty`); or
...
Which tornado commit does the repository build?
====================================================

The `pypi-wheel` repository will build the commit specified in the
...
be something like::
    ...
    wheel-uploader -r warehouse -u $CDN_URL -s -v -w ~/wheelhouse -t macosx tornado $VERSION
    wheel-uploader -r warehouse -u $CDN_URL -s -v -w ~/wheelhouse -t manylinux1 tornado $VERSION
...
``tornado`` is the root name of the wheel(s) to download / upload, and
``4.4.1`` is the version to download / upload.
...
``tornado-4.4.1-`` from http://wheels.scipy.org to ``~/wheelhouse``,
